...
export function ETSidebar({categories}) {
  ...
  return (
    <div className='flex flex-col justify-between shadow-2xl shadow-zinc-900' >
      <ul className="flex flex-row lg:flex-col px-2 pt-5 gap-3 lg:gap-10 lg:h-[50rem] mb-5 w-full lg:w-80 scroll_width">
        ...
        {categories.map((item, index) => (
            ...
            >
              <button 
                ...
                className={ item.category_id == category_id ? 'border rounded-2xl px-2 py-3 bg-neutral-500 w-full text-center text-white font-bold flex': 'w-full hover:text-white hover:bg-black px-2 py-3 font-bold flex' }
              >
                {item.CategoryName}
              </button>
            </li>
